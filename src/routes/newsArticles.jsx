...
export async function loader() {
  const response = await fetch('https://gnews.io/api/v4/search?q=example&lang=en&country=in&max=10&apikey=')
...
export default function NewsArticles() {
  ...
  return (
    <>
      ...
      <div
        id="blog-container"
        className="container lg:container  m-auto sectionThree grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-1"
      >
        {data.articles.map((article, index) => (
          <div
            key={article.id || index}
            className=" max-w-64 m-5 cursor-pointer p-3 border rounded-md border-slate-400 flex flex-col  align-middle"
          >
